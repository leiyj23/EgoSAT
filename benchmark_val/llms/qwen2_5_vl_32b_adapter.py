...
"""
Qwen2.5-VL task adapter (for your model-agnostic runner/helper).
...
仅做“跑 Qwen2.5-VL-32B”所需的点对点改造（最小改动）：
  - 默认 MODEL_ID 改为 Qwen/Qwen2.5-VL-32B-Instruct（仍可 env/CLI 覆盖）
  - 启用 flash_attention_2（你环境可用）
  - 修正 _infer_model_max_len()：支持 128k context（避免误判为 4096）
  - 支持通过 runner 的“未知参数”传入 --model_id /path/to/local_model_dir
    （runner 会忽略该参数，但 adapter 会从 sys.argv 读取并使用本地目录）
"""
...
import os
import re
import sys  # <<< NEW (for --model_id CLI override)
import json
import time
import math
import subprocess
...
def _load_memory_caption_text(caption_path: str) -> Tuple[str, bool, Optional[str], Optional[str]]:
    path = (caption_path or "").strip()
    if not path:
        return "", False, "missing_caption_cache_path", None
    ...
    try:
        ...
        if not isinstance(obj, dict):
            return "", False, "caption_cache_not_json_object", None
        caption_text = str(obj.get("caption_text", "") or obj.get("caption", "") or "").strip()
        api_status = str(obj.get("api_status", "") or "").strip() or None
        return caption_text, bool(caption_text), None, api_status
    except Exception as exc:
        return "", False, repr(exc), None


def _memory_dense_window(sample: Dict[str, Any], memory: Dict[str, Any]) -> Optional[Tuple[float, float, float]]:
    dense = memory.get("dense_window", {}) if isinstance(memory.get("dense_window"), dict) else {}
    if not dense:
        return None
    end_sec = _safe_float(dense.get("end_sec"), _safe_float(sample.get("t_eval_rel", sample.get("t_eval", 0.0)), 0.0))
    start_sec = _safe_float(dense.get("start_sec"), max(0.0, end_sec - _safe_float(dense.get("duration_sec"), 10.0)))
    end_sec = max(0.0, end_sec)
    start_sec = max(0.0, min(start_sec, end_sec))
    return start_sec, end_sec, max(0.0, end_sec - start_sec)


def _inject_memory_caption(sample: Dict[str, Any], base_prompt: str) -> Tuple[str, Dict[str, Any], Optional[Tuple[float, float, float]]]:
    if not _env_bool("MEMORY_MODE", False):
    ...
    if not isinstance(memory, dict) or not memory.get("enabled"):
        return base_prompt, {
            "enabled": False,
            "setting_name": os.environ.get("MEMORY_SETTING_NAME", "").strip(),
    ...
    dense = memory.get("dense_window", {}) if isinstance(memory.get("dense_window"), dict) else {}
    caption_path = str(sparse.get("caption_cache_path", "") or memory.get("caption_cache_path", "") or "").strip()
    caption_text, caption_cache_hit, caption_error, caption_api_status = _load_memory_caption_text(caption_path)

    window = _memory_dense_window(sample, memory)
    t_eval_rel = _safe_float(sample.get("t_eval_rel", sample.get("t_eval", 0.0)), 0.0)
    dense_start = _safe_float(dense.get("start_sec"), window[0] if window else 0.0)
    dense_end = _safe_float(dense.get("end_sec"), window[1] if window else t_eval_rel)
    sparse_end = _safe_float(sparse.get("end_sec"), dense_start)
    keyframe_ts = sparse.get("expected_keyframe_timestamps_sec", [])
    if not isinstance(keyframe_ts, list):
        keyframe_ts = []
    no_lookahead = bool(
        memory.get("no_lookahead", False)
        and dense_end <= t_eval_rel + 1e-6
        and sparse_end <= dense_start + 1e-6
        and all(_safe_float(ts, sparse_end + 1.0) <= sparse_end + 1e-6 for ts in keyframe_ts)
    )

    diag: Dict[str, Any] = {
        "enabled": True,
        "setting_name": memory.get("setting_name"),
        "caption_cache_hit": bool(caption_cache_hit),
        "caption_used": False,
        "caption_missing": True,
    ...
    if caption_api_status is not None:
        diag["caption_api_status"] = caption_api_status
    if caption_error:
        diag["caption_error"] = caption_error

    if not caption_text:
        if _env_bool("MEMORY_FAIL_ON_MISSING_CAPTION", False):
        ...
        return base_prompt, diag, window
    ...
    memory_block = (
        ...
        "Use it only as past context and do not treat it as future evidence.\n"
        f"Memory setting: {setting_name}\n"
        f"Sparse memory window: {_sec_label(sparse.get('start_sec', ''))}s to {_sec_label(sparse.get('end_sec', ''))}s.\n"
    ...
    return memory_block + (base_prompt or "").strip(), diag, window


def _cli_arg_value(flag: str) -> Optional[str]:
    """
    Parse extra args passed to runner.py (runner ignores unknown args).
    Example:
      python runner.py --mode open --model_id /path/to/model
    """
    try:
...
def _parse_vision_enabled_from_argv(default: bool = True) -> bool:
    ...
    Since this adapter is usually imported and executed inside runner.py
    within the same Python process, reading sys.argv here allows:
        python runner.py --mode cand --vision off
    """
    v = _cli_arg_value("--vision")
    if v is None:
        return bool(default)
...
def _important_lines_for_all_tasks(lines: List[str]) -> List[str]:
    ...
    for i, ln in enumerate(lines):
        ...
        if not s:
            continue
        if (
            s.startswith("[Sparse memory")
            or s.startswith("This memory summarizes")
            or s.startswith("Memory setting:")
            or s.startswith("Sparse memory window:")
            or s.startswith("Caption:")
            or s.startswith("[Current task]")
        ):
            keep.append(ln)
            continue
...
def _infer_model_max_len(tokenizer, model) -> int:
    ...
    if tokenizer is not None:
        try:
            ...
            # allow long context (e.g., 128k)
            if 1024 <= ml <= 131072:
                maxlen = ml if maxlen is None else min(maxlen, ml)
        except Exception:
            pass
...
def _encode_inputs_with_autofit(
    ...
    *,
    ...
    if not vision_enabled:
        ...
        if tokenizer is not None:
            try:
                prompt_tok_len = int(len(tokenizer.encode(prompt, add_special_tokens=False)))
            except Exception:
                prompt_tok_len = None
        ...
        encode_diag: Dict[str, Any] = {
            ...
            "tries": [{"frames": 0, "input_len": int(in_len), "vision_enabled": False}],
            "chosen_frames": 0,
            "chosen_input_len": int(in_len),
        }

        moved = {k: v.to(device) for k, v in inputs.items() if isinstance(v, torch.Tensor)}
        return moved, 0, str(prompt), encode_diag
...
class _Qwen25VLAdapter:
    def __init__(self) -> None:
        # Allow CLI override:
        #   python runner.py ... --model_id /path/to/local/Qwen2.5-VL-32B-Instruct
        cli_model_id = _cli_arg_value("--model_id") or _cli_arg_value("--qwen_model_id")  # <<< NEW
        self.model_id = (cli_model_id or _env("MODEL_ID", "Qwen/Qwen2.5-VL-32B-Instruct"))  # <<< MOD (default 32B)
        ...
        # Enable flash attention 2 (your env has it)
        attn_impl = _env("ATTN_IMPL", "flash_attention_2")  # <<< NEW
        ...
        # vision on/off (same style as the OpenRouter blind ablation)
    ...
    # -------------------------
    # (Below: your original implementation unchanged)
    # -------------------------
    ...
    def _generate_one(
        self, prompt_text: str, frames: Optional[np.ndarray], *, vision_enabled: Optional[bool] = None
    ) -> Tuple[str, List[str], List[float], Optional[float], Optional[float], int, Dict[str, Any]]:
        # NOTE: This function is very long; unchanged from your original.
        ...
        if cand_conf_enabled and cand and (self.tokenizer is not None) and subkind in {"ms_pred_cand", "ms_rtrv_cand", "sh_pred_cand"}:
            def _build_cand_conf_prompt(orig: str, opts: Dict[str, Dict[str, str]]) -> str:
                first = ""
                try:
            ...
            prompt_conf = _build_cand_conf_prompt(prompt_after, opts_map)
            ...
            attempt_diag["latency_sec_generate"] = float(time.time() - tcc0)
            ...
            if (not letter) and isinstance(probe, dict) and probe.get("ok") and self.cand_conf_force_fallback:
    ...
    def _rewrite_out_path_for_cand_conf(self, out_path: Path) -> Path:
        """
        Ensure cand_conf outputs do NOT overwrite existing /cand/ results.
        Rewrite .../<task>/cand/... -> .../<task>/cand_conf/...
        Also rewrite filename __cand.json -> __cand_conf.json when present.
        """
        s = out_path.as_posix()
        # If runner already routed to cand_conf, do nothing.
        if "/cand_conf/" in s:
            return out_path
        repls = [
            ("/ms_pred/cand_full/", "/ms_pred/cand_conf/"),
            ("/ms_pred/cand/",      "/ms_pred/cand_conf/"),
            ("/ms_rtrv/cand_full/", "/ms_rtrv/cand_conf/"),
            ("/ms_rtrv/cand/",      "/ms_rtrv/cand_conf/"),
            ("/sh_pred/cand_full/", "/sh_pred/cand_conf/"),
            ("/sh_pred/cand/",      "/sh_pred/cand_conf/"),
            ("/sh_pred_full/cand_full/", "/sh_pred_full/cand_conf/"),
            ("/sh_pred_full/cand/",      "/sh_pred_full/cand_conf/"),
        ]
        s2 = s
        for a, b in repls:
            if a in s2:
        ...
        p2 = Path(s2)
        name = p2.name
        # filename rewrite (best-effort)
        if "__pred__cand_full.json" in name:
            p2 = p2.with_name(name.replace("__pred__cand_full.json", "__pred__cand_conf.json"))
        elif "__pred__cand.json" in name:
            p2 = p2.with_name(name.replace("__pred__cand.json", "__pred__cand_conf.json"))
        elif "__cand_full.json" in name:
            p2 = p2.with_name(name.replace("__cand_full.json", "__cand_conf.json"))
        elif "__cand.json" in name:
            p2 = p2.with_name(name.replace("__cand.json", "__cand_conf.json"))
        elif name.endswith(".json") and ("cand_conf" not in name):
            # fallback: append a suffix so we don't overwrite anything
    ...
    def run(self, *args, **kwargs) -> Path:
        ...
        # NEW: reroute output for cand_conf so it doesn't overwrite /cand/
        ...
        # vision-off ablation: do not touch the video at all
        ...
        for s in samples:
            ...
            memory_window: Optional[Tuple[float, float, float]] = None
            if _env_bool("MEMORY_MODE", False):
                prompt_text, memory_diag, memory_window = _inject_memory_caption(s, prompt_text)

            if memory_window is not None:
                ws_rel, we_rel, lookback_used = memory_window
            else:
            ...
            frame_count = _safe_int(
                dense_diag.get("num_frames", os.environ.get("MEMORY_DENSE_NUM_FRAMES", self.num_frames)),
                self.num_frames,
            ) if memory_window is not None else int(self.num_frames)
            frame_count = max(1, int(frame_count))

            if self.vision_enabled:
                try:
                    frames, _ = _load_frames_decord(video_path, ws_rel, we_rel, frame_count)
                except Exception as e:
                    healed = self._try_self_heal_clip(video_path, qs, e)
                    if healed:
                        frames, _ = _load_frames_decord(video_path, ws_rel, we_rel, frame_count)
            ...
            t0 = time.time()
            #resp_text, gen_tokens, gen_token_probs, sent_logp, mean_logp, frames_used, gen_diag = self._generate_one(prompt_text, frames)
            ...
            # NEW: for cand_conf (letter-only), store parsed.ans for convenience
            #if self._active_pred_flavor == "cand_conf" and task in {"ms_pred", "ms_rtrv", "sh_pred", "sh_pred_full"}:
            ...
            rec = {
                ...
                "diagnostics": {
                    ...
                    "frames_extracted": int(frames.shape[0]) if isinstance(frames, np.ndarray) else 0,
                    "frames_used": int(frames_used),
                    "model_max_len": int(self.model_max_len),
                    "max_input_tokens": int(self.max_input_tokens),
                    "clip_dur_sec": float(clip_dur),
                    "prompt_and_encoding_debug": gen_diag,
                    "memory": memory_diag if _env_bool("MEMORY_MODE", False) else {"enabled": False},
                },
                "raw": None,
            }
