...
DEFAULT_OUT_DIR = Path("outputs/sft/roi_timechat_mixed5_cand")
OUT_DIR = Path(os.environ.get("OUT_DIR", str(DEFAULT_OUT_DIR))).expanduser()
...
def _resize_keep_aspect_no_crop(
    ...
    # if already <= target, still optionally round to mult (but keep change minimal)
...
def _assert_roi_cache_norm(roi_cache: Dict[str, Any]) -> None:
    # roi_cache created by _build_roi_cache_for_indices: {"frames":[...], "frame_w":..., "frame_h":...}
    ...
    for fr in frames:
        ...
        for key in ["hand_boxes", "left_box", "right_box"]:
            ...
            if v is None:
                continue
            # hand_boxes is list of boxes; left/right_box is a box
            if key == "hand_boxes":
...
def _debug_print_vision_tokens(
    *,
    ...
    if igt is not None and hasattr(igt, "shape"):
        try:
            # expected shape: (num_images, 3) with (t, h, w)
...
def load_timechat_roi_model_and_processor(device: torch.device) -> Tuple[torch.nn.Module, Any]:
    ...
    # NEW: try requested attn impl first; fallback to sdpa
    load_err = None
    model = None
    ...
    # record effective impl for printing
...
def build_inputs_for_one(
    ...
    # NEW: resize per-frame, keep aspect, NO crop, NO pad
    # This preserves norm ROI coordinates (0..1) without any change.
    ...
    if USE_DTD_IN_TRAIN and roi_path:
        ...
        if rp.exists() and DROP_METHOD.lower().startswith("roi"):
            ...
            # NEW: assert ROI cache is truly normalized when coord == "norm"
    ...
    # NEW: print per-frame visual token info (before moving tensors)
    try:
