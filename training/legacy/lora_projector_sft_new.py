...
OUT_DIR_ROI = Path(os.environ.get("OUT_DIR_ROI", "outputs/sft/roi_timechat_mixed7_stateheavy_resume")).expanduser()
OUT_DIR_BASE = Path(os.environ.get("OUT_DIR_BASE", "outputs/sft/timechat_mixed7_stateheavy_resume")).expanduser()

# Previous training output dirs (used to auto-find latest checkpoint to continue training)
PREV_OUT_DIR_ROI = Path(os.environ.get("PREV_OUT_DIR_ROI", "/path/to/previous/roi_timechat_adapter")).expanduser()
PREV_OUT_DIR_BASE = Path(os.environ.get("PREV_OUT_DIR_BASE", "/path/to/previous/timechat_adapter")).expanduser()
...
# NEW: select OUT_DIR / PREV_TRAIN_OUT_DIR by TIMECHAT_USE_ROI_MODEL
...
_PREV_OUT_DIR_OVERRIDE = os.environ.get("PREV_TRAIN_OUT_DIR", "").strip()
PREV_TRAIN_OUT_DIR = (
    Path(_PREV_OUT_DIR_OVERRIDE).expanduser()
    if _PREV_OUT_DIR_OVERRIDE
    else (PREV_OUT_DIR_ROI if TIMECHAT_USE_ROI_MODEL else PREV_OUT_DIR_BASE)
)
