...
One-click evaluation for multi-step prediction (ms_pred) in CAND mode only.

Task setting:
- Each anchor event yields a group of 3 queries (n_steps=3), at leads 8/16/24 seconds BEFORE anchor start.
- We evaluate:
  (1) Accuracy by step_idx / lead_sec (0/1/2 -> 8/16/24)
  ...
  (3) Token-prob confidence (ONLY for open-source outputs with gen_tokens/gen_token_probs):
      ...
      - mean conf over valid/correct predictions ONLY when the answer is inferred by OPTION LETTER
             (i.e., conf_source == "token_ans")
  (4) Monotonic diagnostics per anchor group (CONF ONLY, token-derived ONLY):
...
[NEW - optional cand_conf mode]
- Use --mode cand_conf to evaluate predictions produced by cand_conf adapter outputs.
...
- Output directory becomes: ~/benchmark_val/score/<MODEL_NAME>/ms_pred_cand_conf/

[NEW - linear trend slope beta]
- For anchor groups where ALL 3 steps are correct AND all 3 conf available:
    r = 1,2,3 (step rank corresponding to step_idx 0,1,2)
    fit c = alpha + beta r
  report beta (mean/median/std, fraction beta<0),
  and also beta_z where c is z-scored within each group.

[NEW - cand_conf correct/wrong mean confidence]
- In cand_conf mode, for each step rank (0/1/2), additionally report:
...
Inputs:
  ~/benchmark_val/testllm/<MODEL_NAME>/ms_pred/cand/*.json
  OR (cand_conf):
  ~/benchmark_val/testllm/<MODEL_NAME>/ms_pred/cand_conf/*.json

Outputs:
  ~/benchmark_val/score/<MODEL_NAME>/ms_pred_cand/{summary.json, details.jsonl, teacher_cache.json}
  OR (cand_conf):
  ~/benchmark_val/score/<MODEL_NAME>/ms_pred_cand_conf/{summary.json, details.jsonl, teacher_cache.json}

Run:
  MODEL_NAME=qwen2_5_vl_7b python3 ms_pred_eval.py
  MODEL_NAME=qwen2_5_vl_7b python3 ms_pred_eval.py --mode cand_conf
...
# Default is cand; may be overridden in main() by --mode cand_conf
EVAL_MODE = "cand"  # {"cand","cand_conf"}

PRED_DIR = Path.home() / "benchmark_val" / "testllm" / MODEL_NAME / "ms_pred" / "cand"
OUT_DIR = Path.home() / "benchmark_val" / "score" / MODEL_NAME / "ms_pred_cand"
...
def _get_cand_conf_pcond_map(ps: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    Expected path (as in your example):
...
# Linear trend slope
# -------------------------
def _linear_slope_beta(x: List[float], y: List[float]) -> Optional[float]:
...
def _zscore(vals: List[float]) -> Optional[List[float]]:
    if not vals:
        return None
    n = len(vals)
    mu = sum(vals) / float(n)
    var = sum((v - mu) * (v - mu) for v in vals) / float(n)
    if var <= 0:
        return None
    sd = var ** 0.5
...
def _init_step_bucket() -> Dict[str, Any]:
    return {
        ...
        "conf_cnt_wrong": 0,         # NEW
        "conf_sum_wrong": 0.0,       # NEW
        ...
        "conf_cnt_letter_wrong": 0,  # NEW
        "conf_sum_letter_wrong": 0.0,# NEW

        # NEW: cand_conf-specific confidence stats
...
def eval_ms_pred_cand() -> Dict[str, Any]:
    _ensure_dir(OUT_DIR)
    ...
    lead_values_seen: Dict[int, set] = {0: set(), 1: set(), 2: set()}

    group_track: Dict[str, Dict[int, Dict[str, Any]]] = {}

    _log(f"MS_PRED({EVAL_MODE.upper()}): start processing {len(pred_files)} pred files")
    ...
    for fi, pf in enumerate(pred_files, 1):
        ...
        if fi == 1 or (fi % PRED_LOG_EVERY_FILES == 0):
            _log(f"MS_PRED: file {fi}/{len(pred_files)} -> {pf.name} (samples={total_samples}, correct={correct_total})")

        try:
            pred = json.loads(pf.read_text(encoding="utf-8"))
        except Exception as e:
            _log(f"MS_PRED: pred json load failed: {pf.name} err={repr(e)}")
            continue
        num_files_loaded += 1
        ...
        if qpath in queryset_cache:
            ...
            try:
                ...
                _log(f"MS_PRED: queryset load failed: {qp.name} err={repr(e)}")
                continue
        ...
        for ps in psamples:
            ...
            if total_samples == 1 or (total_samples % SAMPLE_LOG_EVERY == 0):
                _log(f"MS_PRED: samples processed={total_samples}, correct={correct_total}, valid={valid_samples}, invalid={invalid_samples}")
            ...
            lead_sec = gt_s.get("lead_sec", None)
            ...
            # fallback by lead_sec (8/16/24)
            if step_idx is None and isinstance(lead_sec, (int, float)):
                l = float(lead_sec)
                if abs(l - 8.0) < 1e-6:
                    step_idx = 0
            ...
            if isinstance(lead_sec, (int, float)):
                lead_values_seen[step_idx].add(float(lead_sec))
            ...
            # ---- original token-derived answer+conf ----
            if has_tokens:
            ...
            if not pred_ans and pred_method != "token_multi":
                if OPENROUTER_API_KEY and options4:
                    ...
                    pred_method = "teacher"
                    _log(
                        "MS_PRED: need teacher_map_ans because ANS unresolved | "
            ...
            # ---- NEW: cand_conf mode overrides confidence using p_cond[ANS] ----
            if EVAL_MODE == "cand_conf":
                if pred_ans and pred_ans in {"A", "B", "C", "D"}:
                    ...
                    if pcond_map is not None and pred_ans in pcond_map:
                        ...
                        # reuse existing statistics buckets as "token_ans" (letter-derived),
                        # but record the true mode in details
                        conf_source = "token_ans"
            ...
            # original token-derived conf (token_ans + token_verbnoun)
            ...
            # token-derived conf ONLY when answer is inferred by option letter (token_ans)
            ...
            # NEW: cand_conf-specific confidence stats
            if EVAL_MODE == "cand_conf" and is_valid and (conf_token is not None) and (conf_tokenprob_mode == "cand_conf_pcond"):
            ...
            detail = {
                ...
                "step_idx": int(step_idx),
                "lead_sec": float(lead_sec) if isinstance(lead_sec, (int, float)) else None,
    ...
    for si in [0, 1, 2]:
        ...
        # NEW: cand_conf-specific summary
        ...
        lead_list = sorted(list(lead_values_seen[si])) if lead_values_seen[si] else []
        ...
        step_entry = {
            ...
            "tokenprob_conf_mean_over_wrong_token_derived": conf_mean_w,              # NEW
            "tokenprob_conf_count_over_wrong_token_derived": conf_cnt_w,              # NEW
            ...
            "tokenprob_conf_mean_over_wrong_token_derived_letter_matched": conf_mean_l_w,  # NEW
            "tokenprob_conf_count_over_wrong_token_derived_letter_matched": conf_cnt_l_w,  # NEW

            # NEW: cand_conf-mode confidence summary
    ...
    for gid, mp in group_track.items():
        ...
        x = [1.0, 2.0, 3.0]
        y = [c0, c1, c2]
        b = _linear_slope_beta(x, y)
        if b is not None:
            betas.append(float(b))

        yz = _zscore(y)
        if yz is not None:
            bz = _linear_slope_beta(x, yz)
            if bz is not None:
                betas_z.append(float(bz))
    ...
    trend = {
        "definition": "For each anchor group with 3/3 correct and 3/3 conf: fit c = alpha + beta r, r in {1,2,3} for step_idx {0,1,2}. beta<0 indicates decreasing trend.",
        "groups_used_all3_correct_with_conf": int(groups_all3_correct_with_conf),

        "beta_mean": _mean(betas),
        "beta_median": _median(betas),
        "beta_std": _std(betas),
        "beta_neg_fraction": (float(sum(1 for b in betas if b < 0.0) / len(betas)) if betas else None),

        "beta_z_mean": _mean(betas_z),
        "beta_z_median": _median(betas_z),
        "beta_z_std": _std(betas_z),
        "beta_z_neg_fraction": (float(sum(1 for b in betas_z if b < 0.0) / len(betas_z)) if betas_z else None),
    }
    ...
    summary = {
        ...
        "task": f"ms_pred_{EVAL_MODE}",
        "pred_dir": str(PRED_DIR),
        "out_dir": str(OUT_DIR),
        "eval_mode": EVAL_MODE,
        "confidence_mode": ("cand_conf_pcond" if EVAL_MODE == "cand_conf" else "gen_token_prob_or_verbnoun"),
...
def main() -> None:
    global MODEL_NAME, EVAL_MODE, PRED_DIR, OUT_DIR, OUT_SUMMARY, OUT_DETAILS, OUT_CACHE

    ap = argparse.ArgumentParser(description="Evaluate ms_pred in cand or cand_conf mode.")
    ap.add_argument(
        "--mode",
        choices=["cand", "cand_conf"],
        default="cand",
        help="cand: evaluate ms_pred/cand (default). cand_conf: evaluate ms_pred/cand_conf and use cand_conf p_cond confidence.",
    )
    ap.add_argument("--flavor", choices=["cand", "cand_conf"], default=None, help="Alias for --mode.")
    ap.add_argument("--model_name", default=None, help="Override MODEL_NAME without changing the environment.")
    ap.add_argument("--runs_root", type=Path, default=None, help="Infer pred_dir as <runs_root>/<model_name>/ms_pred/<flavor>.")
    ap.add_argument("--pred_dir", type=Path, default=None, help="Override prediction directory.")
    ap.add_argument("--out_dir", type=Path, default=None, help="Override output directory.")
    args = ap.parse_args()
    if args.model_name:
        MODEL_NAME = str(args.model_name).strip()
    EVAL_MODE = str(args.flavor or args.mode).strip()

    # switch directories based on mode (cand_conf is sibling of cand)
    if args.pred_dir is not None:
        PRED_DIR = args.pred_dir.expanduser()
    elif args.runs_root is not None:
        PRED_DIR = args.runs_root.expanduser() / MODEL_NAME / "ms_pred" / EVAL_MODE
    elif EVAL_MODE == "cand_conf":
        PRED_DIR = Path.home() / "benchmark_val" / "testllm" / MODEL_NAME / "ms_pred" / "cand_conf"
    else:
        PRED_DIR = Path.home() / "benchmark_val" / "testllm" / MODEL_NAME / "ms_pred" / "cand"
    ...
    if args.out_dir is not None:
        ...
        OUT_DIR = Path.home() / "benchmark_val" / "score" / MODEL_NAME / "ms_pred_cand_conf"
    else:
        OUT_DIR = Path.home() / "benchmark_val" / "score" / MODEL_NAME / "ms_pred_cand"
    ...
    for si in ["0", "1", "2"]:
        ps = s["per_step"][si]
        lead_seen = ps.get("lead_values_seen")
        lead_show = lead_seen[0] if isinstance(lead_seen, list) and len(lead_seen) == 1 else lead_seen

        base_msg = (
            f"step={si} lead={lead_show} | "
    ...
    tr = s.get("trend", {})
    print(f"Groups used                                          : {tr.get('groups_used_all3_correct_with_conf')}")
    print(f"beta_mean / median / std                             : {tr.get('beta_mean')} / {tr.get('beta_median')} / {tr.get('beta_std')}")
    print(f"beta_neg_fraction                                    : {tr.get('beta_neg_fraction')}")
    print(f"beta_z_mean / median / std                           : {tr.get('beta_z_mean')} / {tr.get('beta_z_median')} / {tr.get('beta_z_std')}")
    print(f"beta_z_neg_fraction                                  : {tr.get('beta_z_neg_fraction')}")
